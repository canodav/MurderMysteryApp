...
function mapCharacters(characters: Array<any>): Array<Character> {
    return characters.map(character => {
        return createCharacter(character.id, character.name, character.description, null);
    })
}
...
function mapCharacterStories(characterStories: Array<any>) : Array<CharacterStory> {
    return characterStories.map(characterStory => {
        let facts = mapFacts(characterStory.facts);
        return createCharacterStory(characterStory.character_id, facts, characterStory.extraInfo)
    })
...
function mapFacts(facts: Array<any>) : Array<Fact> {
 ...
 games = stories.map((story) => {
    ...
    return createGame(story.id, story.name, story.description, characters, acts)
})


console.log(games[0].characters);
